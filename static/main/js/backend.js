...
const movieSelection = document.getElementById('movie'); // Ensure movieSelect is defined here
...
function refreshSeat() {
    const movie_title = movieSelection.options[movieSelection.selectedIndex].id;
    ...
    contactAPI("/occupied/", { movie_title }).then(data => {
        ...
        const LS_movie = movieSelection.options[movie_index].textContent;
            ...
            if (seat_LocalStorage !== null) {
                seat_LocalStorage.forEach((seat, index) => {
                    if (occupied_seat.includes(seat)) {
                        ...
                        localStorage.setItem("selectedSeats", JSON.stringify(seat_LocalStorage)); // Ensure to stringify before setting back to localStorage
                    }
                });
            }
        }
